...
// @updateURL    https://github.com/Quakearea/Beeline-ai/raw/main/src/main.user.js
// @downloadURL  https://github.com/Quakearea/Beeline-ai/raw/main/src/main.user.js
// @supportURL   https://github.com/Quakearea/Beeline-ai/issues
// ==/UserScript==
...
function getCurrentQuestion() {
    try {
        ...
        if (!questionType) {
            const t1 = document.querySelector('.common-danxuan-tag p');
            const t2 = document.querySelector('.common-duoxuan-tag p');
            const t3 = document.querySelector('.common-zhuguan-tag p');
            const t4 = document.querySelector('.common-panduan-tag p');
            if (t1 && t1.textContent.includes('单选')) questionType = '单选题';
            else if (t2 && t2.textContent.includes('多选')) questionType = '多选题';
            else if (t3 && t3.textContent.includes('主观')) questionType = '主观题';
            else if (t4 && t4.textContent.includes('判断')) questionType = '判断题';
        }
        ...
        if (questionType === '单选题' || questionType === '判断题') {
            ...
            if (radioGroup) {
                ...
                if (!questionType) {
                    const texts = Array.from(radios).map(r => (r.querySelector('.label')?.textContent || r.textContent || '').trim());
                    const hasJudgeWords = texts.some(t => /对|错|正确|错误|true|false/i.test(t));
                    if (hasJudgeWords) questionType = '判断题';
                }
            }
    ...
    try {
        ...
        if (questionType === '单选题') {
            ...
            if (radioGroup) {
                const radios = radioGroup.querySelectorAll('.el-radio');
                for (let radio of radios) {
                    const labelText = radio.querySelector('.index-name')?.textContent.trim() || '';
                    const label = (labelText.match(/[A-Z]/i) || [])[0]?.toUpperCase();
                    if (label && selectedOptions.includes(label)) {
            ...
            if (checkboxGroup) {
                ...
                checkboxes.forEach(checkbox => {
                    const labelText = checkbox.querySelector('.index-name')?.textContent.trim() || '';
                    const label = (labelText.match(/[A-Z]/i) || [])[0]?.toUpperCase();
                    if (label && selectedOptions.includes(label)) {
                        const input = checkbox.querySelector('input.el-checkbox__original');
                        const inner = checkbox.querySelector('.el-checkbox__inner');
